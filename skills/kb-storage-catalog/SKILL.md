...
| PDF (scanned) / image | read via the enrichment pass (needs a vision-capable reader), or OCR | transcription, or metadata-only |
...
Reference implementation: [`references/cataloger.ts`](references/cataloger.ts) - implements the **local-disk backend** end to end (an S3/GCS backend swaps the walk/fetch for list+fetch calls, ~30 lines; everything downstream is unchanged). Rich-file extraction and the Phase 2 enricher are built in, and the bar is **capability, not specific libraries: every Word doc, Excel, PDF (scanned included), and image in the corpus must actually get read.** Extractors are optional deps with graceful metadata-only fallback and a logged install hint - the cataloger's header lists the current defaults; swap in whatever the box supports as long as the bar holds. Enrichment configuration (endpoint, reader, concurrency) lives in the cataloger's header flags; scanned PDFs and images additionally need a vision-capable reader configured there. Its contract: resumable manifest (keyed on extractor version + enricher version + model, so config changes regenerate), per-file failures logged (never fatal) and retried next run - an enrichment failure still writes the mechanical entry and re-enriches on the next run - stale entries for deleted files removed, per-directory wikilinked `index.md` tree regenerated every run so `kb lint` stays clean. **After changing extractors bump `EXTRACTOR_VERSION`; after changing the enrichment contract bump `ENRICHER_VERSION`** (or pass `--force`) so already-cataloged files regenerate - otherwise the manifest skips them.

## Phase 1 - build the discovery catalog (mechanical)
...
## Phase 2 - optional enrichment (content reading pass)

Additive; the keyword catalog works without it. The enrichment pass **reads each file's actual content** and upgrades its entry: a real `description`, concept `tags`, a content-based `sensitivity`, and - where the file warrants it - durable facts distilled into the body (the `kb-author` distill workflow). The bar: an enriched entry can answer questions about the file; an un-enriched one only matches its keywords.

The reference cataloger runs this pass **inline**: one reading call per file over its extracted content (validated strict JSON, one retry, mechanical fallback on failure) inside the same resumable run - never by an agent pulling files into its own context. The reader is configured in the cataloger's header flags and runs fully local if the box requires it (air-gapped works); scanned PDFs and images need a vision-capable reader, which is what makes photo/scan corpora searchable at all. The reading prompt forbids copying credentials found inside files into any field.
